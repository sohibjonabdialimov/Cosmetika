...
function addNewBook() {
  uploadImages(globalImageUrl).then(res => {
    ...
    if (Object.values(saveObj).every((item) => item)) {
      fetch('https://cosmetika-58c07-default-rtdb.firebaseio.com/cosmetika.json', {
          method: 'POST',
...
function getAllBooks() {
  ...
  fetch('https://cosmetika-58c07-default-rtdb.firebaseio.com/cosmetika.json').then(res => {
    ...
    .then(res => {
      // if(document.getElementById('dashboard__product').classList.includes('dashboard__active')){
      ...
      // }
      // else if(document.getElementById('dashboard__blogs').classList.includes('dashboard__active')){
...
document.getElementById('dashboard__product').addEventListener('click', () => {
  renderPagination(allBooks.length);
  renderHtmlElements(choppedPagination(allBooks));
});
document.getElementById('dashboard__blogs').addEventListener('click', () => {
  renderPagination(allBooks1.length);
  renderHtmlElements(choppedPagination(allBooks1));
});
  ...
  let result = books.map((item, index) => {
    ...
    <div class="book__item">
            ...
            <div class="book__desc">
              <h4>${item.title} </h4>
              <p class="book__author">by &nbsp  <span>Nimadur</span> , Nimadur</p>
...
function editSaveFunc() {
  editUploadImages(editImageUrl).then(res => {
    ...
    if (Object.values(editObj).every((item) => item)) {
      fetch(`https://cosmetika-58c07-default-rtdb.firebaseio.com/cosmetika/${globalEdit.id}.json`, {
          method: 'PUT',
...
function deleteBook(ok) {
  ...
  fetch(`https://cosmetika-58c07-default-rtdb.firebaseio.com/cosmetika/${findedElement.id}.json`, {
      method: 'DELETE',
    }).then(res => {
      if (!res.ok) throw new Error('Xatolik bor')
      return res.json();
...
// Dashboard js
document.querySelectorAll('.dashboard__list li').forEach(item => {
  item.addEventListener('click', () => {
    document.querySelectorAll('.dashboard__list li').forEach(it => {
      it.classList.remove('dashboard__active');
    })
    item.classList.add('dashboard__active');
  })
})
